...
| Release provenance | Version tag or manual | Creates a deterministic source archive, CycloneDX source SBOM, checksums, and GitHub artifact attestation. |
...
The root `gradlew` and `gradlew.bat` launch Gradle 8.14.5 for both managed roots:
...
`6f74b601422d6d6fc4e1f9a1ab6522f642c2fdcbc15ae33ebd30ba3d7198e854`. The checked-in wrapper
JAR has SHA-256
`7d3a4ac4de1c32b59bc6a4eb8ecb8e612ccd0cf1ae1e99f66902da64df296172`. Both values are checked
...
## Known build-tool advisories

The 2026-09-06 resolution still has 85 advisory matches across selected Gradle build,
instrumentation, and plugin components. The four-package Kotlin/Android release runtime graph had
no OSV or Grype match in the same proof. Build tools execute on the build host, so they remain a
supply-chain constraint even when absent from the published AAR and Maven runtime.

The only direct affected component is Kotlin Gradle plugin 2.3.21 under
`GHSA-r937-wjx7-w2jp`. The first fixed 2.4.20 line was prerelease during the scan. An isolated
Android Gradle Plugin 9.3.2 and Gradle 9.5.0 migration still left 83 selected advisory matches and
therefore was not adopted. The complete 47-ID unresolved set, direct/transitive reachability,
available fixes, and upgrade experiment are recorded in [security-scan.md](security-scan.md) and
its machine-readable evidence index.

Until a compatible stable toolchain resolves the direct advisory, CI and release commands keep
the Gradle build cache disabled. Dependabot surfaces stable candidates, but no update is merged
without compilation, package, dependency-resolution, and managed binding checks. Private
transitive versions inside the Android Gradle Plugin are not forced outside Google's tested graph.
The scheduled scan retains a fresh selected-component Gradle SBOM and Grype result on each run;
its current known findings stay visible rather than being suppressed as if the build graph were
clean.
