...
Binding rows describe the currently verified binding surface. The canonical manifest and each
binding's conformance test decide parity; copied raw bytes alone do not count as typed support.
...
| Applications/keys | Create/delete application, delegated create/query/delete, key settings/version, AES key changes, initialize/finalize/roll key set; offline AES EncK and create/delete/delegated-configuration DAM MAC generation | Exact command/chaining fixtures, public AN12696 create known answers, NXP Reader Library 7.11 source-parity configuration-input fixtures, independent OpenSSL delete/configuration MAC answers, authenticated Card workflows, and preflight range failures | Delegated option-6 SetConfiguration card payload lacks an authoritative supplied layout |
...
| MIFARE Classic compatibility | RestoreTransfer between two value files using Plain or MAC communication; offline AES MFCLicenseMAC over an explicit structurally checked license and caller-supplied sector secrets | Exact target/source command data and Standard-AES CMAC-chain fixtures; independent OpenSSL license-MAC answer; boundary/provider/exception failures | CreateMFCMapping and RestrictMFCUpdate remain unavailable because the supplied material does not define their option-dependent typed layouts; sector-secret subfields remain caller-owned |
...
| Python | Typed Python 3.10+ sync/async `ctypes` facade with managed, expert raw and all offline operations | 28 tests on Python 3.10 and 3.12, strict mypy, Ruff, generated parity, clean wheel install and known-answer smoke test | Native library is supplied separately; other Python runtimes are not recorded |
| Node/TypeScript | Typed N-API facade with managed, expert raw and offline workers plus asynchronous reader/provider rendezvous | 20 native integration tests, strict TypeScript, generated parity and package verification on Node 26 | Node 20/22/24 and Linux/Windows addon execution remain unrecorded |
...
| Android | IsoDep adapter and verified AAR with ARM64, ARMv7 and x86_64 JNI/C/C++ runtimes | All three ABI cross-build/export/dependency checks, 16 KiB ARM64 alignment, release lint, Android-test/example compilation and AAR hash verification | No current-revision emulator or physical-NFC execution; API 23 and ARMv7 runtime remain unqualified |
...
- AuthenticatePDC; the supplied implementation references an unavailable internal specification,
  leaves part of the terminal response uninterpreted, and provides no authoritative EV3 vectors.
- MIFARE Classic mapping and update-restriction card commands; their option-dependent typed
  layouts are absent from the supplied material.
